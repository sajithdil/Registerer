define(['backbone'],function(Backbone){
	var remove = function(rowId,router)
	{
			...
			var URL = "/"+$.cookie('emp_no')+"/permission/employees/"+$('#delPerGrid').jqGrid('getCell', rowId, 'PermissionID') ;
			var options = {
			  url: URL,
				colModel:[
			  ...
			  //console.log("fired here");
				...
				$('#mess').dialog({
					...
					buttons : {
						"Yes": function() {
							$.post(URL + "/remove/"+$('#perGrid').jqGrid('getCell', id, 'emp_no'),function(res){
								//close the grids
								$('#mess').dialog("close");
								$('#mess').remove();
