define(['backbone'],function(Backbone){
	var remove = function(rowId)
	{
			...
			var URL = "/"+$.cookie('emp_no')+"/employee/department/manager/"+$('#delEmpDepGrid').jqGrid('getCell', rowId, 'dept_no') ;
			var options = {
			  url: URL,
			  postData: {
					val: "search"
			  ...
			  //console.log("fired here");
			 	var temp = "<div id='mess'>Remove employee from department?</div>";
				...
				$('#mess').dialog({
					...
					buttons : {
						"Yes": function() {
							//console.log(rowId);
							$.post(URL + "/removemanager/"+$('#perGrid').jqGrid('getCell', id, 'emp_no'),function(res){
								//console.log($('#perGrid').jqGrid('getCell', rowId, 'emp_no'));
								//close the grids
								$('#mess').dialog("close");
								$('#mess').remove();
