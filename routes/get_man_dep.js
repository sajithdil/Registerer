/*
 BL to do GET for getting the managers from the department for the selected permission
 */
...
//define([],function (){
	var editEmp = function(req, res){
		...
		{	var page = parseInt(req.query["page"]);
			...
			seq.query("SELECT * FROM employees e INNER JOIN dept_manager l on e.emp_no = l.emp_no WHERE l.dept_no ='" + dept+"'"+ "LIMIT "+noOfRows+" OFFSET " + off).success(function(emp){
				//console.log(emp);
				seq.query("SELECT COUNT(*) FROM employees e INNER JOIN dept_manager l on e.emp_no = l.emp_no WHERE l.dept_no ='" + dept+"'").success(function(c){
					var row = new Array();
					var cou=0;
					try{
